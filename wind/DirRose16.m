...
wind_count(:,2) = wind_count(:,1)/size(direction,1);
w = ceil(max(wind_count(:,2))*100/m)/100;
po = wind_count(:,2)*wn/w+0.08;   %相当于po/(m*w)
...
%% text
[max_feq,ind] = max(wind_count(:,2));
max_dir = dir_ang(ind)*180/pi;
title('风向玫瑰图','unit','normalized','position',[1 1],...
    'FontSize',20);
text(1.18,1,{['最大风向频率：',num2str(roundn(max_feq*100,-2)),'%'];...
